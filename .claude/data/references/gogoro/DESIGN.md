    ...
    display:
      {
        ...
        weight: 700,
        lineHeight: 1.05,
        use: "Full-bleed hero statements, tight tracking",
      }
    ...
    button-cta:
      {
        ...
        padding: "0 24px",
        font: "14px / 600",
        use: "Account / login / primary site CTA, 40px height",
      }
    button-marketing:
      {
        type: button,
        ...
        padding: "14px 28px",
        font: "16px / 600",
        use: "Hero CTA over black / photo backgrounds",
      }
    button-outline:
      {
        type: button,
        ...
        padding: "14px 28px",
        font: "16px / 600",
        use: "Lower-priority action, 1px #dee2e6 border",
      }
    input-default:
      {
        type: input,
        ...
        padding: "12px 16px",
        font: "16px / 400",
        use: "Account / form input; focus border #0074ff",
      }
    card-spec:
      {
        type: card,
        ...
        radius: "16px",
        padding: "24px",
        use: "Product / spec card on light surface",
      }
    card-dark:
      {
        type: card,
        ...
        fg: "#ffffff",
        radius: "16px",
        use: "On-black storytelling card with electric-blue accent",
      }
...
1. **The product is the hero; the UI is the chassis.** Marketing surfaces recede into black so the Smartscooter can carry the page. _UI implication:_ Default hero background to `#000000` with full-bleed product media; keep chrome minimal and let one headline + one CTA do the work. Don't decorate around the product.

2. **Blue is current, not color.** Electric blue (`#0074ff`) and cyan (`#28c3ff`) are rationed to energy moments — links, active states, battery/charge graphics. _UI implication:_ If a surface has more than ~10% blue area, you're overspending the voltage. Use grayscale for structure; reserve blue for where energy is literally or interactively present.

3. **Numbers are the argument.** Range, swap-time, station count, kilometers traveled — Gogoro proves claims with metrics, not adjectives. _UI implication:_ Render hero statistics large and bold (`36px+`/700). Never pair a numeric claim with hype copy; the number is the persuasion.

4. **The network must feel as reliable as a gas pump.** GoStation UI is utility, not marketing — accuracy and legibility over flourish. _UI implication:_ Station/battery-availability states must be unambiguous and live-accurate; use the charge palette consistently; never animate availability in a way that implies a state that isn't real.

5. **Engineered restraint.** Graphik, tight letterspacing, soft 12px radii, flat surfaces with luminous accents — everything reads as designed by engineers with taste. _UI implication:_ Prefer value-step layering over heavy shadows; keep one radius system; let typography weight (not size inflation) carry hierarchy.

6. **Two languages, both native.** English and Traditional Chinese are authored in parallel as first-class voices. _UI implication:_ Never machine-translate one from the other; keep the Graphik + `PingFang TC` / `Noto Sans TC` stack so both render with equal care. Simplified Chinese is never acceptable on TW surfaces.

## 13. Personas

_Personas are fictional archetypes informed by publicly described Gogoro user segments (Taiwanese urban commuters and the licensed-partner ecosystem), not individual people._
