...
<Droplet className="text-brand-500 size-6" strokeWidth={1.5} />;
```
...
```css
.wave-back {
  animation: waveX 6.5s linear infinite;
}
.wave-front {
  animation: waveX 4s linear infinite reverse;
}
@keyframes waveX {
  from {
...
.liquid-group {
  transition: transform 1s cubic-bezier(0.4, 1.4, 0.5, 1);
}
```
...
```json
{
  ...
  "lib_used": null,
  "tokens_cited": [
    "foundations.tokens.brand-500",
    "foundations.tokens.brand-300",
    "foundations.tokens.radius-step",
